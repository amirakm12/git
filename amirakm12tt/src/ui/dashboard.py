...
class DashboardServer:
    ...
    def __init__(self, config: SystemConfig, orchestrator):
        ...
        # Background tasks
        self.background_tasks = {}  # Add missing attribute
    ...
    async def initialize(self):
        ...
        if not AIOHTTP_AVAILABLE:
            logger.warning("aiohttp not available, dashboard features disabled")
            return
    ...
    async def start(self):
        ...
        if self.app is None:
            logger.warning("Dashboard not initialized, skipping start")
            return
        ...
        try:
            ...
            self.site = web.TCPSite(
                self.runner,
                self.config.ui.dashboard_host,
                self.config.ui.dashboard_port
            )
